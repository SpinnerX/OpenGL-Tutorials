 ...
 * 
 * @param DiffuseMaps
 * @note Reason for wanting some ways to setting diffuse colors of an object for each individual fragments.
 * @note A system where you cant retrieve color values based on fragment position on the object (?)
 * @note Basically this is just a texture where the image is just wrapped around an object we can index for unique color values per fragment
 * @note In lit scenes, this usually is referred to as diffuse ma (generally how 3D artists call them before PBR) since texture image represents all of object's diffuse colors.
 * @note To get an idea reference to basics/figures/DiffuseMapFigure.png
 * 
 * @param MoreThingsToNOTE
 * @note One thing to Note is originally did not plan to add a skybox to this demo
 * @note But added the skybox to it anyways
 * 
*/
...
void DiffuseLightingMapsExample(GLFWwindow* window){
    ...
    Shader lightShader("basics/shaders/lightingMapsTutorial-01_1/light.vert", "basics/shaders/lightingMapsTutorial-01_1/light.frag");
    Shader cubeShader("basics/shaders/lightingMapsTutorial-01_1/cube.vert", "basics/shaders/lightingMapsTutorial-01_1/cube.frag");
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        // lightShader.Set("objectColor", cubeColor); // This is how you set the actual cube color to BLUE, that is reflecting off of the light source
