...
/**
 * @example Example #8 - Coordinate System
 ...
 * 
 * @param Local_Space (or Obj Space)
 * @note Where local space means coordinates of your object relative to it's local origin point
 * @note Initially they're coordinates your object begins
 * 
 * @param World_Space 
 * @note Next would be transforming local coordinates to world-space coordinates which are coordinates in respect to the larger world.
 * @note Coordinates relative to global origin of the world, together with many objects placed in worlds origin
 * 
 * @param View_Space (or Eye Space)
 * @note Then transforming world coordinates to view-space coordinates in a way each coordinate is seen from camera/views pov.
 * 
 * @param Clip_Space
 * @note Coordinates in view-space to project in clip-space
 * @note Clip processed to the -1.0 and 1.0 range, determining which vertices will end up on screen
 * @note Projection to clip-space coordinate can add perspective if using perspective projection.
 * 
 * @param Screen_Space
 * @note Transform clip coords to screen coords in process called viewport
 * @note Transforming coords from -1.0 and 1.0 to coordinate range defined by glViewport
 * @note Resulting coordinates are sent rasterized turning into fragments.
 * 
 * @name Resulting output of this example
 * @note You should this cube rotate
 * @note Showcasing how you interact with shaders to rotate your texture
*/
...
void CoordinateSystemExample(GLFWwindow* window){
    printf("Example 09 -- Coordinate Systems\n");
